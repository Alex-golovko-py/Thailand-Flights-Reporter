...
# ========== прокси ==========
PROXY_URL = "socks5://setere:ZX90jwdsg3408he@135.181.250.175:12703"
...
def get_clickhouse_client(config):
    """Получение клиента ClickHouse (подключается напрямую, без прокси)"""
...
def get_monthly_report(config: Dict) -> str:
    """Получение месячного отчета (агрегированные данные за 4 недели)"""
    client = get_clickhouse_client(config)
    try:
        moscow_tz = pytz.timezone(config['report']['timezone'])
        now_moscow = datetime.now(moscow_tz)
        ...
        month_start = current_monday
        month_end = current_monday + timedelta(days=27)  # 4 недели = 28 дней - 1
        ...
        # Собираем данные по каждой неделе и агрегируем
        weekly_data_list = []
        top_countries_agg = {}
        prev_top_countries_agg = {}
        peak_day_counter = {}
        ...
        for i in range(4):
            ...
            # Агрегируем рейсы и вместимость
            month_data['flights'] += week_data['flights']
            month_data['capacity'] += week_data['capacity']
            month_data['prev_flights'] += week_data.get('prev_flights', 0)
            
            # Агрегируем ТОП-3 страны
            for country in week_data['top_countries']:
                country_name = country['country']
                capacity_country = country['capacity']
                if country_name in top_countries_agg:
                    top_countries_agg[country_name] += capacity_country
                else:
                    top_countries_agg[country_name] = capacity_country
            
            # Агрегируем данные прошлой недели для трендов
            for country, capacity in week_data['prev_countries_capacity'].items():
                if country in prev_top_countries_agg:
                    prev_top_countries_agg[country] += capacity
                else:
                    prev_top_countries_agg[country] = capacity
            
            # Агрегируем общую международную вместимость
            month_data['total_international_capacity'] += week_data.get('total_international_capacity', 0)
            
            # Находим самый загруженный день недели за месяц
            if week_data.get('peak_day') and week_data.get('peak_day_flights', 0) > 0:
                day = week_data['peak_day']
                flights = week_data['peak_day_flights']
                if day in peak_day_counter:
                    peak_day_counter[day] += flights
                else:
                    peak_day_counter[day] = flights
        
        # Определяем самый загруженный день за месяц
        if peak_day_counter:
            month_data['peak_day'] = max(peak_day_counter, key=peak_day_counter.get)
            month_data['peak_day_flights'] = peak_day_counter[month_data['peak_day']]
        
        # Формируем ТОП-3 страны за месяц
        sorted_countries = sorted(top_countries_agg.items(), key=lambda x: x[1], reverse=True)[:3]
        month_data['top_countries'] = [
            {'country': country, 'capacity': capacity} 
            for country, capacity in sorted_countries
        ]
        
        # Формируем данные прошлой недели для трендов (агрегированные)
        month_data['prev_countries_capacity'] = prev_top_countries_agg
        
        # Прогноз на следующий месяц (простой тренд по 4 неделям)
        if len(weekly_data_list) >= 3:
            total_current_month = month_data['flights']
            prev_month_flights = sum(w.get('prev_flights', 0) for w in weekly_data_list[:3]) / 3 if len(weekly_data_list) >= 3 else total_current_month
            trend = (total_current_month - prev_month_flights) / prev_month_flights if prev_month_flights > 0 else 0
            month_data['forecast_next'] = max(0, int(total_current_month * (1 + trend)))
            month_data['forecast_change'] = round(((month_data['forecast_next'] - total_current_month) / total_current_month) * 100, 1) if total_current_month > 0 else 0
        
        # Формируем отчет в том же стиле, что и недельный
        report = f"📊 Monthly FORECAST: Phuket\n\n"
        
        # Основные метрики за месяц
        report += f"🗓 Forecast for the month: {month_start_str} - {month_end_str}\n"
        report += f"<b>{month_data['flights']}</b> flights arriving in Phuket with total capacity of <b>{month_data['capacity']:,}</b> seats.\n\n"
        
        # Самый загруженный день
        if month_data['peak_day'] and month_data['peak_day_flights'] > 0:
            report += f"📅 Busiest day of the month: <b>{month_data['peak_day']}</b> ({month_data['peak_day_flights']} flights)\n"
        
        # Пустая строка между пиковым днем и прогнозом
        if month_data['peak_day'] and month_data['peak_day_flights'] > 0 and month_data['forecast_next']:
            report += f"\n"
        
        # Прогноз на следующий месяц
        if month_data['forecast_next'] and month_data['forecast_change'] is not None:
            forecast_symbol = "📈" if month_data['forecast_change'] > 0 else "📉" if month_data['forecast_change'] < 0 else "➡️"
            if month_data['forecast_change'] > 0:
                forecast_text = f"+{month_data['forecast_change']}%"
            elif month_data['forecast_change'] < 0:
                forecast_text = f"{month_data['forecast_change']}%"
            else:
                forecast_text = "0%"
            report += f"🔮 Next month forecast: <b>{month_data['forecast_next']}</b> flights ({forecast_symbol} {forecast_text})\n"
        
        # Сравнение с сезонными нормами (среднее за месяц)
        seasonal = config['report'].get('seasonal_averages', {})
        high_season_avg = seasonal.get('high_season', 850) * 4  # умножаем на 4 недели
        low_season_avg = seasonal.get('low_season', 620) * 4
        
        high_season_diff = calculate_percent_change(month_data['flights'], high_season_avg)
        low_season_diff = calculate_percent_change(month_data['flights'], low_season_avg)
        
        report += f"\n📈 Comparison with seasonal norms:\n"
        report += f"— Deviation from high season average: <b>{high_season_diff}%</b>\n"
        report += f"— Deviation from low season average: <b>{low_season_diff}%</b>\n\n"
        
        # ТОП-3 направления за месяц
        exclude_thailand = config['report'].get('exclude_thailand_from_top', False)
        if exclude_thailand:
            report += f"✈️ TOP-3 destinations for the month (excluding Thailand):\n"
        else:
            report += f"✈️ TOP-3 destinations for the month (where guests come from):\n"
        
        total_international_capacity = month_data['total_international_capacity']
        if total_international_capacity == 0:
            total_international_capacity = sum(c['capacity'] for c in month_data['top_countries']) if month_data['top_countries'] else 1
        
        country_emoji_map = config.get('country_emoji', {})
        
        for country_data in month_data['top_countries']:
            country = country_data['country']
            capacity_country = country_data['capacity']
            ...
            # Тренд по стране
            prev_capacity = prev_top_countries_agg.get(country, 0)
            trend = calculate_trend(capacity_country, prev_capacity)
...
async def scheduled_weekly_report():
    ...
    try:
        ...
        await bot.send_message(chat_id=LEADS_CHAT_ID, text=report_text)
...
async def weekly_report(message: types.Message):
    ...
    try:
        report_text = get_weekly_report(CONFIG)
        await message.answer(report_text, reply_markup=get_main_keyboard())
...
async def full_report(message: types.Message):
    ...
    if LEADS_CHAT_ID:
        ...
        await message.answer(lead_message)
...
async def main():
    ...
    logger.info("Starting bot with proxy...")
    logger.info(f"Proxy URL: {PROXY_URL}")
    
    # Создаём сессию с прокси (простой таймаут 180 секунд)
